...
int obtenerEspacioLibre(int flag){
    for (int i=0; i < MAX; i++){
        if (listaAgenda[i].estado != Usado){  //usado es cte=1
   ...
   // SEPARO POR GRUPOS DE EDADES Y DIFINO LAS FILAS SEGUN EL GRUPO MAYOR
    ...
    for (int i = 0; i < arrayLenght; i++) {
        if (listadob[i]->edad < 19) {
                ...
                if (contadores[0]>filas){//compara los numeros mayores de filas
                    filas= contadores[0];
                    }
            }
    ...
    for (int i = filas; i >= 0; i--) {
        if (i==0){//verifica si faltan personas por imprimir
        ...
        for (int j = 0; j < grupos; j++) {//define en que grupo imprime una barra
            if(contadores[j]< i){
                printf("     ");
            }else{
                printf("  %c  ",178);
